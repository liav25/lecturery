...
class VideoProcessor:
    ...
    def __split_video(self, input_file, output_dir, start_time, end_time, segment_time="00:10:00"):
        if not os.path.exists(output_dir):
            os.makedirs(f"{output_dir}/transcriptions")
        else:
            logging.info(f"Output directory {output_dir} already exists. Skipping creation.")
            return True
    ...
    def __process_video_file(self, video_file, output_dir, transcriptions_dir, language):
        ...
        transcription_file = os.path.join(transcriptions_dir, f"{os.path.splitext(video_file)[0]}_transcription.txt")
    ...
    def process_videos(
        ...
        for video_file in os.listdir(output_dir):
            if video_file.endswith(".mp4"):
                self.__process_video_file(video_file, output_dir, transcriptions_dir, language)
